...
const Feature = ({
  ...
  return (
    ...
    >
      {index < 4 && (
        <div className="opacity-0 group-hover/feature:opacity-100 transition duration-200 absolute inset-0 h-full w-full bg-gradient-to-t from-primary-50 dark:from-gray-800 to-transparent pointer-events-none" />
      )}
      {index >= 4 && (
      ...
      <div className="text-lg font-bold mb-2 relative z-10 px-10">
        ...
        <span className="group-hover/feature:translate-x-2 transition duration-200 inline-block text-gray-950 dark:text-gray-100">
          {title}
        </span>
      </div>
      <p className="text-sm text-gray-900 dark:text-gray-300 max-w-xs relative z-10 px-10">
        {description}
      </p>
    </div>
  );
