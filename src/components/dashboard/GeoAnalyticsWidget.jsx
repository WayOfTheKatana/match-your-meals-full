...
const fetchGeoAnalytics = async (userId) => {
  ...
  // Get country analytics - fetch all records and group manually
  const { data: countryViews, error: countryError } = await supabase
    .from('recipe_views')
    .select('country_name')
    .in('recipe_id', recipeIds)
    .not('country_name', 'is', null);
  ...
  // Get region analytics - fetch all records and group manually
  const { data: regionViews, error: regionError } = await supabase
    .from('recipe_views')
    .select('region')
    .in('recipe_id', recipeIds)
    .not('region', 'is', null);
  ...
  // Group countries manually
  const countryCount = {};
  (countryViews || []).forEach(view => {
    if (view.country_name) {
      countryCount[view.country_name] = (countryCount[view.country_name] || 0) + 1;
    }
  });
  
  // Group regions manually
  const regionCount = {};
  (regionViews || []).forEach(view => {
    if (view.region) {
      regionCount[view.region] = (regionCount[view.region] || 0) + 1;
    }
  });
